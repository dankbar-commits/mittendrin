...
{document}""")
    ...
    def process_pdf(
        ...
        Args:
            ...
            source_name: Human-readable source name
            output_folder: Optional folder to save individual item JSONs
            split_items: If True, save each item as a separate JSON file
        ...
        # Step 5: Optionally save individual items to folder
        if output_folder:
            ...
            if split_items:
                ...
                for idx, (item_id, item) in enumerate(adapter_data.itemsRecord.items()):
                    item_file = output_dir / f"item_{idx+1:03d}_{item_id}.json"
                ...
                print(f"✓ Saved {len(items)} individual items to {output_folder}/")
            else:
                # Save complete adapter data to folder
                folder_output_path = output_dir / Path(output_path).name
                ...
                print(f"✓ Saved output to {folder_output_path}")
...
def main():
    ...
    parser.add_argument(
        "-f", "--output-folder",
        help="Output folder to save JSON files (optional)"
    )
    parser.add_argument(
        "--split-items",
        action="store_true",
        help="Save each extracted item as a separate JSON file in output folder"
    )
    parser.add_argument(
        "--adapter-name",
        default="pdf-import",
